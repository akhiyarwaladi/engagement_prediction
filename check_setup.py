...
def check_setup():
    ...
    if data_file.exists():
        try:
            ...
            print(f"✅ Data file found: {len(df)} posts")
            checks.append(True)
        except ImportError:
            print(f"⚠️  Data file found but pandas not installed")
    ...
    if modules_ok:
        print(f"✅ All source modules found")
        checks.append(True)
    else:
        checks.append(False)
    ...
    if dirs_ok:
        ...
        print(f"⚠️  Some directories missing (will be created automatically)")
