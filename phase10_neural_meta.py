...
Replace Ridge meta-learner with MLP
...
print("="*90)
print(" "*15 + "PHASE 10.5: NEURAL NETWORK META-LEARNER")
...
# ============================================================================
# Experiment: Different neural network meta-learners
# ============================================================================

print("[EXPERIMENT] Testing neural network meta-learners...")
print()

meta_configs = [
    {'name': 'MLP (16 neurons, 1 layer)', 'hidden': (16,), 'alpha': 0.01, 'lr': 'constant', 'lr_init': 0.001},
    {'name': 'MLP (32 neurons, 1 layer)', 'hidden': (32,), 'alpha': 0.01, 'lr': 'constant', 'lr_init': 0.001},
    {'name': 'MLP (16-8, 2 layers)', 'hidden': (16, 8), 'alpha': 0.01, 'lr': 'constant', 'lr_init': 0.001},
    {'name': 'MLP (32-16, 2 layers)', 'hidden': (32, 16), 'alpha': 0.01, 'lr': 'constant', 'lr_init': 0.001},
    {'name': 'MLP (64-32-16, 3 layers)', 'hidden': (64, 32, 16), 'alpha': 0.001, 'lr': 'adaptive', 'lr_init': 0.001},
    {'name': 'MLP (32-16-8, 3 layers)', 'hidden': (32, 16, 8), 'alpha': 0.01, 'lr': 'adaptive', 'lr_init': 0.001},
]
...
oof_preds_scaled = meta_scaler.fit_transform(oof_preds)
test_preds_scaled = meta_scaler.transform(test_preds)
...
for config in meta_configs:
    meta = MLPRegressor(
        ...
        learning_rate_init=config['lr_init'],
        max_iter=1000,
        random_state=42,
        early_stopping=True,
    ...
    meta.fit(oof_preds_scaled, y_train_log)

    y_pred_log = meta.predict(test_preds_scaled)
...
print()
print(f"[BEST NEURAL META] {best_config}: MAE={best_mae:.2f}")
print()
...
if best_mae < 45.10:
    ...
    model_filename = f'models/phase10_5_neural_meta_{timestamp}.pkl'

    model_package = {
        'phase': '10.5_neural_meta_learner',
        'mae': best_mae,
...
if best_mae < 45.10:
    ...
    print(f"   Neural meta-learner did not improve (Ridge remains optimal)")
