...
if not Path(baseline_path).exists():
    print(f"[WARN]  Baseline dataset not found, creating from scratch...")
...
print(f"   Phase 4a (NOW)   | {X_combined.shape[1]:<8} | {mae_test:6.2f}     | {r2_test:.4f}    | +IndoBERT [MODEL]")
print("-" * 80)
...
print(f"\n[MODEL] IndoBERT contribution (vs Phase 2):")
...
if mae_test <= 95:
    print(f"   [OK] MAE Target: ACHIEVED ({mae_test:.2f} <= 95)")
else:
    print(f"   [WARN]  MAE Target: NOT MET ({mae_test:.2f} > 95)")
    gap = mae_test - 95
    print(f"      Gap: {gap:.2f} likes (need Phase 4b with ViT)")

if r2_test >= 0.25:
    print(f"   [OK] R² Target: ACHIEVED ({r2_test:.4f} >= 0.25)")
else:
    print(f"   [WARN]  R² Target: NOT MET ({r2_test:.4f} < 0.25)")
    gap = 0.25 - r2_test
...
print("   1. [OK] IndoBERT embeddings (768-dim)")
if USE_PCA:
    print("   2. [OK] PCA reduction (768 -> 50 dims)")
print("   3. [OK] Same robust preprocessing & ensemble")
...
if r2_test > 0.2006:
    print(f"\n   [DONE] IndoBERT improves R² by {improvement_r2_phase2:.1f}%!")
else:
    print(f"\n   ℹ[EMOJI]  IndoBERT provides {abs(improvement_r2_phase2):.1f}% R² change")
