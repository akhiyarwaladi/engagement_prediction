#!/usr/bin/env python3
"""
PHASE 10.30: RATIO FEATURES
Hypothesis: Relative proportions might be informative
Test: caption/hashtag ratio, word/caption ratio, hashtag/mention ratio
...
print("="*90)
print(" "*10 + "PHASE 10.30: RATIO FEATURES (Relative Proportions)")
...
# NEW: Ratio features (relative proportions)
print("[ENGINEER] Creating ratio features...")
df['word_per_caption'] = df['word_count'] / (df['caption_length'] + 1)
df['hashtag_per_word'] = df['hashtag_count'] / (df['word_count'] + 1)
df['hashtag_per_caption'] = df['hashtag_count'] / (df['caption_length'] + 1)
df['mention_per_word'] = df['mention_count'] / (df['word_count'] + 1)
df['hashtag_mention_ratio'] = df['hashtag_count'] / (df['mention_count'] + 1)
df['text_visual_ratio'] = df['caption_length'] / (df['file_size_kb'] + 1)

ratio_features = ['word_per_caption', 'hashtag_per_word', 'hashtag_per_caption',
                  'mention_per_word', 'hashtag_mention_ratio', 'text_visual_ratio']
...
visual_features = (metadata_base + ['aspect_ratio', 'resolution_log', 'aspect_ratio_sq',
                   ...
                   cross_interactions + ratio_features)

print(f"[STRATEGY] Phase 10.23 + ratio features")
print(f"   Baseline: {len(baseline_cols)}, BERT PCA: 60, Visual+Cross+Ratios: {len(visual_features)}")
print()
...
print("="*90)
print(f"[RESULT] Phase 10.30: MAE={mae:.2f}, R2={r2:.4f}")
print("="*90)

if mae < 43.64:
    print(f"[CHAMPION] NEW RECORD! Beat Phase 10.23 by {((43.64-mae)/43.64*100):.2f}%!")
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    joblib.dump({'phase': '10.30', 'mae': mae, 'r2': r2, 'ratio_features': ratio_features, 'timestamp': timestamp},
                f'models/phase10_30_ratio_features_{timestamp}.pkl')
else:
