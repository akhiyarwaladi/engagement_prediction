#!/usr/bin/env python3
"""
PHASE 10.16: FINAL PUSH - ONE SHOT OPTIMIZATION
Based on Phase 10.9 winner: Metadata visual features work best!
Target: Beat MAE=44.05 with ONE targeted improvement
ULTRA FAST: Single config, 5-fold CV, 4 models
"""
...
print("="*90)
print(" "*12 + "PHASE 10.16: FINAL PUSH (SINGLE SHOT OPTIMIZATION)")
print(" "*20 + "Target: Beat Phase 10.9 MAE=44.05")
print("="*90)
print()
...
# WINNING FORMULA from Phase 10.9: Metadata only!
# Add ONE smart enhancement: Resolution normalization (log scale works better for skewed distributions)
metadata_features = ['aspect_ratio', 'file_size_kb', 'is_portrait', 'is_landscape', 'is_square']

# Replace raw resolution with log-normalized version (better distribution)
df['resolution_log'] = np.log1p(df['resolution'])
metadata_features.append('resolution_log')

print("[STRATEGY] Phase 10.9 winner + log(resolution) enhancement")
print(f"   Baseline: {len(baseline_cols)} features")
print(f"   BERT: {len(bert_cols)} -> 50 PCA")
print(f"   Metadata (enhanced): {len(metadata_features)} features")
print()
...
# PCA BERT
print("[PCA] Reducing BERT from 768 to 50 dimensions...")
...
print(f"   Variance preserved: {pca_bert.explained_variance_ratio_.sum()*100:.1f}%")
print()
...
print(f"   - BERT PCA: 50")
print(f"   - Metadata: {len(metadata_features)}")
print()
...
# Stacking (5-fold, 4 models - same as Phase 10.9)
...
print("="*90)
print(f"[RESULT] Phase 10.16 Final Push: MAE={mae:.2f}, R2={r2:.4f}")
print("="*90)
print()

if mae < 44.05:
    improvement = (44.05 - mae) / 44.05 * 100
    print(f"[CHAMPION] NEW RECORD! Beat Phase 10.9 by {improvement:.2f}%!")
    print(f"   Phase 10.9: MAE=44.05")
    print(f"   Phase 10.16: MAE={mae:.2f}")
    print(f"   Improvement: {44.05 - mae:.2f} MAE points")
    print()

    # Save model
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    model_filename = f'models/phase10_16_final_push_{timestamp}.pkl'

    model_package = {
        'phase': '10.16_final_push',
        'mae': mae,
        'r2': r2,
        'features_count': X_train.shape[1],
        'enhancement': 'log(resolution)',
    ...
    print(f"[MATCHED] Same performance as Phase 10.9 (MAE=44.05)")
    print(f"   log(resolution) enhancement: No improvement")
else:
    print(f"[RESULT] Phase 10.9 remains champion")
    print(f"   Phase 10.9: MAE=44.05")
    print(f"   Phase 10.16: MAE={mae:.2f}")
    print(f"   Difference: +{mae - 44.05:.2f} MAE (worse)")

print()
print("="*90)
print(" "*25 + "PHASE 10.16 COMPLETE!")
print("="*90)
