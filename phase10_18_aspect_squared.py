#!/usr/bin/env python3
"""
PHASE 10.18: ASPECT RATIO SQUARED
Insight: log(resolution) worked in Phase 10.16
Hypothesis: Polynomial on aspect_ratio might capture non-linear composition effects
Strategy: Metadata + log(resolution) + aspect_ratio²
...
print("="*90)
print(" "*8 + "PHASE 10.18: ASPECT RATIO SQUARED (Non-linear Composition)")
...
df['aspect_ratio_sq'] = df['aspect_ratio'] ** 2    # NEW: Capture non-linear composition

# Keep original aspect_ratio too
enhanced_features = metadata_base + ['aspect_ratio', 'resolution_log', 'aspect_ratio_sq']

print("[STRATEGY] Phase 10.16 + aspect_ratio² (non-linear composition effect)")
...
print(f"     - aspect_ratio (original)")
print(f"     - aspect_ratio_sq (squared - NEW)")
print(f"     - resolution_log (Phase 10.16)")
print()
...
print("="*90)
print(f"[RESULT] Phase 10.18 Aspect Squared: MAE={mae:.2f}, R2={r2:.4f}")
print("="*90)
print()
...
if mae < 43.92:
    ...
    print(f"   Phase 10.18: MAE={mae:.2f}")
    print(f"   Improvement: {43.92 - mae:.2f} MAE points")
    print()

    # Save model
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    model_filename = f'models/phase10_18_aspect_squared_{timestamp}.pkl'

    model_package = {
        'phase': '10.18_aspect_squared',
        'mae': mae,
        'r2': r2,
        'features_count': X_train.shape[1],
        'enhancements': ['log_resolution', 'aspect_ratio_squared'],
    ...
    diff = mae - 43.92
    if diff > 0:
        print(f"   Aspect squared added noise: +{diff:.2f} MAE (worse)")
    else:
        print(f"   Matched performance!")

print()
print("="*90)
print(" "*25 + "PHASE 10.18 COMPLETE!")
print("="*90)
