#!/usr/bin/env python3
"""
PHASE 10.24: BERT PCA 70 COMPONENTS
Phase 10.21 with 60 components: MAE=43.70 (89.9% variance)
Hypothesis: 70 components might preserve ~91% variance, recover more text nuance
...
print("="*90)
print(" "*10 + "PHASE 10.24: BERT PCA 70 COMPONENTS (Even More Text Signal)")
...
print(f"[STRATEGY] Phase 10.23 + BERT PCA 70 (was 60)")
print(f"   Baseline: {len(baseline_cols)}, BERT: {len(bert_cols)} -> 70 PCA (was 60), Visual+Cross: {len(visual_features)}")
print()
...
print(f"   Variance preserved: {variance_preserved*100:.1f}% (was 89.9% with 60)")
print()
...
print(f"[FEATURES] Total: {X_train.shape[1]} features (was 79 with PCA=60)")
print()
...
print("="*90)
print(f"[RESULT] Phase 10.24: MAE={mae:.2f}, R2={r2:.4f}")
print("="*90)

if mae < 43.64:
    print(f"[CHAMPION] NEW RECORD! Beat Phase 10.23 by {((43.64-mae)/43.64*100):.2f}%!")
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    joblib.dump({'phase': '10.24', 'mae': mae, 'r2': r2, 'bert_pca': 70, 'variance': variance_preserved, 'timestamp': timestamp},
                f'models/phase10_24_bert_pca70_{timestamp}.pkl')
else:
