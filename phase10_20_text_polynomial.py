#!/usr/bin/env python3
"""
PHASE 10.20: TEXT FEATURE POLYNOMIAL
Insight: We tested visual polynomial (aspect²) and it worked!
Hypothesis: Text features might also benefit from polynomial transformation
Test: caption_length², word_count², hashtag_count²
...
print("="*90)
print(" "*8 + "PHASE 10.20: TEXT FEATURE POLYNOMIAL (Non-linear Text Effects)")
...
# Base baseline features
baseline_base = ['is_video', 'hour', 'day_of_week', 'is_weekend', 'month']

# Text features to polynomialize
text_features = ['caption_length', 'word_count', 'hashtag_count', 'mention_count']

# BERT
bert_cols = [col for col in df.columns if col.startswith('bert_')]

# Visual features (from Phase 10.18 winner)
...
# NEW: Text Polynomial Features
print("[ENGINEER] Creating text polynomial features...")

df['caption_length_sq'] = df['caption_length'] ** 2
df['word_count_sq'] = df['word_count'] ** 2
df['hashtag_count_sq'] = df['hashtag_count'] ** 2
df['mention_count_sq'] = df['mention_count'] ** 2

# Also test log transform for text (like we did for resolution)
df['caption_length_log'] = np.log1p(df['caption_length'])
df['word_count_log'] = np.log1p(df['word_count'])

text_polynomial = [
    'caption_length_sq', 'word_count_sq', 'hashtag_count_sq', 'mention_count_sq',
    'caption_length_log', 'word_count_log'
]

print(f"   Created {len(text_polynomial)} text polynomial/log features:")
print(f"     - caption_length² & log(caption_length)")
print(f"     - word_count² & log(word_count)")
print(f"     - hashtag_count²")
print(f"     - mention_count²")
print()

# Combine all
baseline_cols = baseline_base + text_features + text_polynomial

print(f"[STRATEGY] Text Polynomial + Phase 10.18 Visual")
print(f"   Baseline: {len(baseline_cols)} features (incl. text poly)")
...
print("="*90)
print(f"[RESULT] Phase 10.20 Text Polynomial: MAE={mae:.2f}, R2={r2:.4f}")
print("="*90)
print()
...
if mae < 43.89:
    ...
    print(f"   Phase 10.20: MAE={mae:.2f}")
    print(f"   Improvement: {43.89 - mae:.2f} MAE points")
    print()

    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    model_filename = f'models/phase10_20_text_polynomial_{timestamp}.pkl'
    model_package = {
        'phase': '10.20_text_polynomial',
        'mae': mae,
        'r2': r2,
        'features_count': X_train.shape[1],
        'text_polynomial': text_polynomial,
    ...
    diff = mae - 43.89
    if diff > 0:
        print(f"   Text polynomial added noise: +{diff:.2f} MAE")

print()
print("="*90)
print(" "*25 + "PHASE 10.20 COMPLETE!")
print("="*90)
