#!/usr/bin/env python3
"""
PHASE 10.28: BERT PCA 80 COMPONENTS
Phase 10.21: 60 components = 89.9% variance
Phase 10.24: 70 components = ~91% variance
Hypothesis: 80 components might preserve ~92% variance, maximum BERT signal
...
print("="*90)
print(" "*8 + "PHASE 10.28: BERT PCA 80 COMPONENTS (Maximum Text Signal)")
...
print(f"[STRATEGY] Phase 10.23 + BERT PCA 80 (maximum text signal)")
print(f"   Baseline: {len(baseline_cols)}, BERT: {len(bert_cols)} -> 80 PCA, Visual+Cross: {len(visual_features)}")
print()
...
# BERT PCA 80 components
print("[PCA] Reducing BERT from 768 to 80 dimensions...")
pca_bert = PCA(n_components=80, random_state=42)
...
print("="*90)
print(f"[RESULT] Phase 10.28: MAE={mae:.2f}, R2={r2:.4f}")
print("="*90)

if mae < 43.64:
    print(f"[CHAMPION] NEW RECORD! Beat Phase 10.23 by {((43.64-mae)/43.64*100):.2f}%!")
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    joblib.dump({'phase': '10.28', 'mae': mae, 'r2': r2, 'bert_pca': 80, 'variance': variance_preserved, 'timestamp': timestamp},
                f'models/phase10_28_bert_pca80_{timestamp}.pkl')
else:
