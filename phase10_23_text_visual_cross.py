#!/usr/bin/env python3
"""
PHASE 10.23: TEXT-VISUAL CROSS INTERACTIONS
Hypothesis: Caption length × aspect ratio might show content-composition alignment
Test: caption × aspect, hashtags × resolution, word_count × file_size
...
print("="*90)
print(" "*8 + "PHASE 10.23: TEXT-VISUAL CROSS INTERACTIONS (Multimodal Synergy)")
...
# NEW: Text × Visual cross interactions
print("[ENGINEER] Creating text-visual cross interactions...")
...
print(f"   Created {len(cross_interactions)} text-visual cross features")
print()
...
print(f"[STRATEGY] Phase 10.21 + text×visual cross interactions")
print(f"   Baseline: {len(baseline_cols)}, BERT PCA: 60, Visual+Cross: {len(visual_features)}")
print()
...
print("="*90)
print(f"[RESULT] Phase 10.23: MAE={mae:.2f}, R2={r2:.4f}")
print("="*90)

if mae < 43.70:
    print(f"[CHAMPION] NEW RECORD! Beat Phase 10.21 by {((43.70-mae)/43.70*100):.2f}%!")
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    joblib.dump({'phase': '10.23', 'mae': mae, 'r2': r2, 'cross_interactions': cross_interactions, 'timestamp': timestamp},
                f'models/phase10_23_text_visual_cross_{timestamp}.pkl')
else:
