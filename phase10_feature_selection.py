#!/usr/bin/env python3
"""
PHASE 10.7: ADVANCED FEATURE SELECTION
Use mutual information and correlation to select best visual + text features
...
print("="*90)
print(" "*15 + "PHASE 10.7: ADVANCED FEATURE SELECTION")
...
print("[EXPERIMENT] Testing feature selection strategies...")
print()
...
print("[FEATURE IMPORTANCE] Computing mutual information scores...")
mi_scores = mutual_info_regression(X_train, y_train_log, random_state=42)
mi_scores_sorted_idx = np.argsort(mi_scores)[::-1]

print(f"   Top 10 features by MI:")
for i in range(min(10, len(mi_scores_sorted_idx))):
    idx = mi_scores_sorted_idx[i]
    if idx < 9:
        fname = baseline_cols[idx]
    elif idx < 59:
        fname = f'bert_pca_{idx-9}'
    else:
        fname = f'aes_pca_{idx-59}'
    print(f"      {i+1}. {fname}: {mi_scores[idx]:.4f}")
print()

# ============================================================================
# Experiment: Different feature selection strategies
# ============================================================================

selection_configs = [
    {'name': 'All 65 features (Phase 9)', 'n_features': 65},
    {'name': 'Top 50 by MI', 'n_features': 50},
    {'name': 'Top 40 by MI', 'n_features': 40},
    {'name': 'Top 30 by MI', 'n_features': 30},
    {'name': 'Top 45 by MI', 'n_features': 45},
    {'name': 'Top 55 by MI', 'n_features': 55},
]
...
for config in selection_configs:
    ...
    if n_features == 65:
        ...
        top_idx = mi_scores_sorted_idx[:n_features]
    ...
    print(f"   {config['name']:<30} MAE={mae:.2f}, R2={r2:.4f}")
...
print()
print(f"[BEST SELECTION] {best_config}: MAE={best_mae:.2f}")
print()
...
if best_mae < 45.10:
    ...
    model_filename = f'models/phase10_7_selection_{timestamp}.pkl'

    model_package = {
        'phase': '10.7_feature_selection',
        'mae': best_mae,
...
if best_mae < 45.10:
    ...
    print(f"   Feature selection did not improve (all 65 features remain optimal)")
