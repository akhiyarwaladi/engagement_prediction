#!/usr/bin/env python3
"""
PHASE 10.22: FILE SIZE SQUARED
Phase 10.18 proved aspect_ratio² works!
Hypothesis: file_size_kb² might capture quality threshold effects
Target: Beat MAE=43.70
"""
...
print("="*90)
print(" "*10 + "PHASE 10.22: FILE SIZE SQUARED (Quality Threshold)")
print(" "*18 + "Target: Beat Phase 10.21 MAE=43.70")
print("="*90)
print()
...
# Phase 10.21 visual features
metadata_base = ['is_portrait', 'is_landscape', 'is_square']
...
# NEW: file_size polynomial
df['file_size_sq'] = df['file_size_kb'] ** 2
df['file_size_log'] = np.log1p(df['file_size_kb'])

visual_features = metadata_base + ['aspect_ratio', 'file_size_kb', 'resolution_log', 'aspect_ratio_sq',
                                    'aspect_x_logres', 'filesize_x_logres', 'aspect_sq_x_logres',
                                    'file_size_sq', 'file_size_log']

print(f"[STRATEGY] Phase 10.21 + file_size² + log(file_size)")
print(f"   Baseline: {len(baseline_cols)}, BERT: {len(bert_cols)} -> 60 PCA, Visual: {len(visual_features)}")
print()
...
print("="*90)
print(f"[RESULT] Phase 10.22: MAE={mae:.2f}, R2={r2:.4f}")
print("="*90)

if mae < 43.70:
    print(f"[CHAMPION] NEW RECORD! Beat Phase 10.21 by {((43.70-mae)/43.70*100):.2f}%!")
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    joblib.dump({'phase': '10.22', 'mae': mae, 'r2': r2, 'timestamp': timestamp},
                f'models/phase10_22_filesize_sq_{timestamp}.pkl')
else:
    print(f"Phase 10.21 remains champion (MAE=43.70)")
print("="*90)
